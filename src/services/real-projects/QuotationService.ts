  ...
  static async generateQuotationHtml(project: RealProject, quotationData: Record<string, any>, washrooms: Washroom[]): Promise<string> {
    ...
    const htmlTemplate = `
      ...
      <body>
                    ...
                    <tbody>
                      ${sortedCategories.map(([category, items]) => {
                        ...
                        return `
                            ...
                            if (!item.isBrandProduct && item.serviceDetails) {
                              return item.serviceDetails.map((service: any) => {
                                ...
                                return `
                                  <tr>
                                    <td>${categoryName}</td>
                                    <td style="padding-left: 24px;">${serviceName} ${unit ? `(${unit})` : ''}</td>
                                    <td style="text-align: right;">₹${formatAmount(serviceMrp)}</td>
                                    <td style="text-align: right;">₹${formatAmount(displayCost)}</td>
                                  </tr>
                                `;
                              }).join('');
                            } else {
                              // For brand products or fixtures
                              return `
                                <tr>
                                  <td>${category}</td>
                                  ...
                                  <td style="text-align: right;">
                                    ...
                                    ${discountPercentage > 0 ? ` <span style="color: #10b981; font-size: 12px;">(${discountPercentage}% off)</span>` : ''}
                                  </td>
                                </tr>
                              `;
                            }
                          ...
                          <tr>
                            <td colspan="3" style="text-align: right; font-weight: 600;">Category Total:</td>
                            <td style="text-align: right; font-weight: 600;">₹${formatAmount(categoryTotal)}</td>
                          </tr>
                        `;
                      }).join('')}
                    </tbody>
        ...
        <div class="summary-box">
            ...
            ${Object.entries(costBreakdown).map(([key, cost]) => {
              ...
              return `
                <div class="price-row cost-category">
                  <span class="cost-category-title">${cost.title}</span>
                  <span class="cost-category-amount">₹${formatAmount(cost.amount)}</span>
                </div>
              `;
            }).join('')}
          ...
          <div style="margin-top: 30px;">
            ...
            <ul style="padding-left: 20px; font-size: 14px;">
              ...
              <li>50% advance payment is required to start the work.</li>
              <li>40% payment is required after completion of civil works and before fixture installation.</li>
              <li>Remaining 10% is due upon project completion.</li>
              <li>Prices are inclusive of all materials, labor, and installation.</li>
              <li>GST is applicable as per government regulations.</li>
              <li>Any additional work or changes outside of this quotation will be charged extra.</li>
            </ul>
          </div>
        </div>
        ...
        <div class="footer">
          <img src="/lovable-uploads/0e4ef5ca-8d0e-4e79-a60b-aceb673a33b7.png" alt="Your Dream Space Logo" />
          <p>Thank you for choosing Purebath Interiotech for your washroom transformation!</p>
          <p>www.yourdreamspace.in | info@yourdreamspace.in | +91 9999999999</p>
        </div>
